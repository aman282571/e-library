...
//storing fileon server end

//mongodb conection
mongoose.connect('mongodb+srv://aman28:Aman28@cluster0.ohyhb.mongodb.net/libraryData?retryWrites=true&w=majority', { useNewUrlParser: true, useUnifiedTopology: true  } ).then(()=>{
  ...
  info.findById(req.params.id,(err,data)=>{
        ...
        else
        {
            ...
            let mailDetails = { 
                ...
                html: `<h3> Hi ${data.name} </h3>${req.body.name} wants to borrow a book named : <b>${data.bookname.toUpperCase()}</b> from you. <br/>Borrower gmail address is 
                ${req.body.email} <br/> Kindly send him a mail regarding this book. `
            }; 
